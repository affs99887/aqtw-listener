...
按格数展示候选缩略图的本地听音浮窗。当前为 **历史记录与拾起音效修复预览版，尚未完成实战准确率验收**。
...
1. 完整解压程序包，双击 `AqtwListener.exe`。无需安装 .NET 或 Go。
...
主窗口按「监听／采音／浮窗／音效库／诊断」分区切换，不再使用长滚动设置页面，也不显示浮窗预览。游戏内浮窗默认鼠标穿透、不抢焦点，位置、宽度、透明度及是否显示名称可在主窗口调整。图片默认从 60 放大到 88，可调范围 60–120。浮窗默认宽 470，高度完全由候选内容决定：结果少时收紧，结果多时展开，减少后再次收紧。固定高度输入及开关已移除，旧配置中的 `height`、`autoHeight` 不再生效。

浮窗、历史、离线结果及图鉴窗口优先展示完整内容，窗口增长最多到所在屏幕工作区；放不下时按完整卡片分页，没有候选滚动条。数量较少的格数组并排展示，候选总数和占比始终按全部结果计算。浮窗使用 **Ctrl+Alt+PgUp / PgDn** 翻页（仅多页结果、监听开启且游戏在前台时注册），也可用主窗口「候选上一页／下一页」；历史、图鉴及离线窗口直接点击翻页按钮。同一批候选的重复匹配不会重置当前页。

「识别历史」在独立窗口显示本次运行最近 100 条有效匹配，记录列表分页，点击查看当时全部候选；窗口高度也随内容伸缩。相邻两秒内候选相同的自动扫描合并，同次点击的初步/最终结果更新为一条记录；独立点击分别保留。「清除当前结果」不会删除历史，「清空历史」也不会清除浮窗。历史仅存内存，退出后清空，不保存音频。离线文件识别和物品图鉴仍在单独窗口展示。

### UU 远程或拖动没有反应

助手与游戏需要运行在同一台被控电脑。如果浮窗显示「鼠标触发 0 次 · 已收到声音」，请保持「声音自动识别」勾选。开启监听且游戏在前台时，新版直接从近期音频中识别，浮窗的「声音分析 N 次」会增加；鼠标事件仍使用事件与按键状态两路检测。

自动识别仅分析有声音的窗口，静音时等待，不累计识别任务；切出游戏、关闭监听或关闭自动识别都会取消正在计算的任务，但不删除已经接受的匹配。静音、分析中、未匹配和识别错误只更新诊断，不覆盖保留的候选。自动模式会分析背景音乐、界面和物品等通过所选设备播放的声音；实验音效库的覆盖范围和准确率限制仍适用。
